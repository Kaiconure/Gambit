...
function arrayContainsAnyStrI(array, ...)
    local searchFor = {...}
    if type(searchFor[1]) == 'table' then
        searchFor = searchFor[1] 
    end

    if isArray(array) and isArray(searchFor) then
        for i = 1, #searchFor do
            local searchItem = type(searchFor[i]) == 'string' and string.lower(searchFor[i])
            if searchItem then
                for j = 1, #array do
                    local arrayItem = type(array[j]) == 'string' and string.lower(array[j])
                    if searchItem == arrayItem then
                        return searchItem
                    end
                end
            end
